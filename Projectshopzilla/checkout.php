...
include 'config.php';
$cart_sess;
 $sql_pdt = "SELECT product.*,cart.quantity as cart_qnty,cart.item as cart_item from product left outer join cart on cart.item=product.id where cart.session='$cart_sess'";
$exec_pdt = mysql_query($sql_pdt);
$num_pdt=mysql_num_rows($exec_pdt);
//echo $num_pdt;
 $sql_cart1 ="SELECT cart.id as cart_id,cart.session,cart.user_uniq,cart.item,cart.quantity as cart_qnty,cart.price,cart.time,product.id as pdt_id,product.name as pdt_name,product.price as pdt_price,product.quantity as pdt_qnty FROM cart LEFT outer JOIN product ON( product.id=cart.item) WHERE session='$cart_sess'"; 
$exec_cart1=mysql_query($sql_cart1);
$i=0;
  while($fetch_cart1=mysql_fetch_assoc($exec_cart1)){;
 $total_price+=$fetch_cart1['cart_qnty'] * $fetch_cart1['pdt_price'];
$i++;}
...
if($_POST['submit']=='Continue')
{
	...
	$dvar['region']=trim($_POST['sregion']);
	...
	/*	elseif($state1=="")
	{
		$msg1="Please select state";
		$response1="error";
	}
*/
	else{
		//echo "hi";
    $sql_insert="INSERT into  billing_details (first_name,last_name,company,company_id,address_1,address_2,city,postcode,country,region,s_firstname,s_lastname,s_company,s_company_id,s_address_1,s_address_2,s_city,s_postcode,s_country,s_region,status,time)VALUES('".$dvar['first_name']."','".$dvar['last_name']."','".$dvar['company']."','".$dvar['company_id']."','".$dvar['address_1']."','".$dvar['address_2']."','".$dvar['city']."','".$dvar['postcode']."','".$country."','".$state."','".$dvar['f_name']."','".$dvar['l_name']."','".$dvar['comp']."','".$dvar['comp_id']."','".$dvar['add_1']."','".$dvar['add_2']."','".$dvar['cit']."','".$dvar['pcode']."','".$count."','".$state."','1','".time()."')";//echo "<br>";
    ...
    if($num_pdt > 0)
     {
      ...
      $sql_place="INSERT into place_order(session,shipping_id,pro_id,total,quantity,status,time)
	   VALUES('".$cart_sess."','".$last_id."','".$fetch_pdt['cart_item']."','".$total_price."','".$fetch_pdt['cart_qnty']."','0','".time()."')";
	   $exec_place=mysql_query($sql_place);
	    if($exec_place)
	    {
		header("location:payment.php?payment=paypal");
	    }
		else
		{
			echo mysql_error();
              ...
              <tr>
                ...
                <tr>
                  <td><span class="required">*</span> Country:</td>
                      ...
                      <td><select class="large-field" name="country" onchange="select_state_registration();" id="slelect_country_id">
                  ...
                  <td><div id="slelect_state_id"> <select name="state" >
                      <option value=""> --- Please Select --- </option>
                    </select></div></td>
                </tr>
                      <!--<option value=""> --- Please Select --- </option>
              ...
              <tr>
                ...
                <tr>
                  <td><span class="required">*</span> Country:</td>
                      ...
                      <td><select class="large-field" name="count" onchange="select_state_registration();" id="slelect_country_id">
                      <option value=""> --- Please Select --- </option>
                       <?php
                    ...
                    <tr>
                  <td><span class="required">*</span> Region / State:</td>
                  <td><div id="slelect_state_id"> <select name="state1" >
                      <option value=""> --- Please Select --- </option>
                    </select></div></td>
                </tr>
                      <!--<option value=""> --- Please Select --- </option>
